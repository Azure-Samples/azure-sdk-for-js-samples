...
async function resourceLinks_listAtSubscription(){
    for await (let item of client.resourceLinks.listAtSubscription()){
        console.log(item);
    }
}
...
async function resourceLinks_listAtSourceScope(){
    ...
    for await (let item of client.resourceLinks.listAtSourceScope(resourceId)){
        console.log(item);
    }
}
...
async function resources_beginDeleteAndWait(){
    //delete reource
    await resources_client.resources.beginDeleteAndWait(resourceGroup,"Microsoft.Compute","","availabilitySets",resourceName,"2019-07-01").then(
        res => {
            console.log(res);
        }
    )
    ...
    //delete reources2
    await resources_client.resources.beginDeleteAndWait(resourceGroup,"Microsoft.Compute","","availabilitySets",resourceName+"2","2019-07-01").then(
        res => {
            console.log(res);
        }
    )
