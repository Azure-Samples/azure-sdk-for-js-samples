import { NetworkManagementClient,IpGroup } from "@azure/arm-network";
...
async function ipGroups_beginCreateOrUpdateAndWait(){
    ...
    await client.ipGroups.beginCreateOrUpdateAndWait(resourceGroup,ipGroupName,parameter).then(
        res => {
            console.log(res);
        }
    )
...
async function ipGroups_get(){
    await client.ipGroups.get(resourceGroup,ipGroupName).then(
        res => {
            console.log(res);
        }
    )
...
async function ipGroups_listByResourceGroup(){
    for await (let item of client.ipGroups.listByResourceGroup(resourceGroup)){
        console.log(item);
    }
}
...
async function ipGroups_beginDeleteAndWait(){
    await client.ipGroups.beginDeleteAndWait(resourceGroup,ipGroupName).then(
        res => {
            console.log(res);
        }
    )
