import { NetworkManagementClient,PublicIPAddress,VirtualNetworkGateway,TroubleshootingParameters,VerificationIPFlowParameters } from "@azure/arm-network";
...
async function virtualNetworksAndSubnetCreate(){
    await client.virtualNetworks.beginCreateOrUpdateAndWait(resourceGroup,virtualnetworkName,{location: "eastus",addressSpace: {addressPrefixes: ['10.0.0.0/16']}}).then(
        res => {
            console.log(res);
        }
    )
    //create subnet
    await client.subnets.beginCreateOrUpdateAndWait(resourceGroup,virtualnetworkName,subnetName,{addressPrefix:'10.0.0.0/24'}).then(
        res => {
            console.log(res);
        }
    )
...
async function networkInterfaces_beginCreateOrUpdateAndWait(){
    ...
    const networkInterface_create = await client.networkInterfaces.beginCreateOrUpdateAndWait(resourceGroup,networkInterfaceName,{location: "eastus",ipConfigurations:[{name: 'MyIpConfig',subnet:{id:subneyId}}]});
        ...
        ipConfigurations: [
            {
                ...
                subnet: {
                    id: "/subscriptions/" + subscriptionId + "/resourceGroups/" + resourceGroup + "/providers/Microsoft.Network/virtualNetworks/" + virtualnetworkName + "/subnets/" + subnetName
                },
                publicIPAddress: {
...
async function networkWatchers_createOrUpdate(){
    await client.networkWatchers.createOrUpdate(resourceGroup,networkWatcherName,{location: "eastus"}).then(
        res => {
            console.log(res);
        }
    )
...
async function networkWatchers_beginGetTroubleshootingAndWait(){
    const parameter:TroubleshootingParameters = {
        targetResourceId: "/subscriptions/" + subscriptionId + "/resourceGroups/" + resourceGroup + "/providers/Microsoft.Network/virtualNetworkGateways/" + virtualNetworkGatewayName,
        ...
        storagePath: "https://" + storageAccountName + ".blob.core.windows.net/troubleshooting"
    };
    await client.networkWatchers.beginGetTroubleshootingAndWait(resourceGroup,networkWatcherName,parameter).then(
        res => {
            console.log(res);
        }
    )
...
async function networkWatchers_beginGetTroubleshootingResultAndWait(){
    await client.networkWatchers.beginGetTroubleshootingResultAndWait(resourceGroup,networkWatcherName,{targetResourceId: "/subscriptions/" + subscriptionId + "/resourceGroups/" + resourceGroup + "/providers/Microsoft.Network/virtualNetworkGateways/" + virtualNetworkGatewayName}).then(
        res => {
            console.log(res);
        }
    )
...
async function virtualMachines_beginCreateOrUpdateAndWait(){
    ...
    await compute_client.virtualMachines.beginCreateOrUpdateAndWait(resourceGroup,vmName,parameter).then(
        res => {
            console.log(res);
        }
    )
...
async function virtualMachineExtensions_beginCreateOrUpdateAndWait(){
    ...
    await compute_client.virtualMachineExtensions.beginCreateOrUpdateAndWait(resourceGroup,vmName,vm_extensionName,parameter).then(
        res => {
            console.log(res);
        }
    )
...
async function networkInterfaces_get(){
    const getResult = await client.networkInterfaces.get(resourceGroup,networkInterfaceName);
...
async function networkWatchers_beginVerifyIPFlowAndWait(){
    ...
    const parameter:VerificationIPFlowParameters = {
        targetResourceId: "/subscriptions/" + subscriptionId + "/resourceGroups/" + resourceGroup + "/providers/Microsoft.Compute/virtualMachines/" + virtualMachineName,
        direction: "Outbound",
        protocol: "TCP",
        localPort: "80",
        remotePort: "80",
    ...
    await client.networkWatchers.beginVerifyIPFlowAndWait(resourceGroup,networkWatcherName,parameter).then(
        res => {
            console.log(res);
        }
    )
