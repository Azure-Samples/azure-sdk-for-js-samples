import { WebSiteManagementClient,Certificate } from "azure-arm-appservice";
...
async function certificates_createOrUpdate(){
    ...
    await client.certificates.createOrUpdate(resourceGroup,name,parameter).then(
        res => {
            console.log(res);
        }
    )
...
async function certificates_get(){
    await client.certificates.get(resourceGroup,name).then(
        res => {
            console.log(res);
        }
    )
...
async function certificates_listByResourceGroup(){
    for await (let item of client.certificates.listByResourceGroup(resourceGroup)){
        console.log(item);
    }
}
...
async function certificates_update(){
    await client.certificates.update(resourceGroup,name,{password: "SWsSsd__233$Sdsds#%Sd!"}).then(
        res => {
            console.log(res);
        }
    )
...
async function certificates_delete(){
    await client.certificates.delete(resourceGroup,name).then(
        res => {
            console.log(res);
        }
    )
